...
class Upload{
    ...
    public function __construct($settings,$infiles=[],$target_dir=''){
        ...
        $this->db = new mysqli("localhost", 'root','root','memes');
    ...
    public function doUploads($infiles=[],$target_dir=""){
        ...
        foreach($this->ufiles as $upfile){
            ...
            if (move_uploaded_file($upfile["tmp_name"], $target_file)) {
                logg('SUCCESS');
                $result = $this->add_to_db($time,$imageFileType,$upfile["name"]);
                if($result['success']){
    ...
    private function add_to_db($time,$type,$original_filename,$uid=0){
        $dir = $this->targetDir;
        $thumbs = $this->targetThumbs;
        $ip = $_SERVER['REMOTE_ADDR'];
        $sql = "INSERT INTO `images` VALUES (NULL, {$uid}, '{$time}', '{$type}', '{$time}', '{$dir}', '{$thumbs}', '', '{$ip}','{$original_filename}');";
